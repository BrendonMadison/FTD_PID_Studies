...
//Struct for defining calorimeter geometry
	...
	void SetMSValues(double mspl)
	{
        ...
        double GetX0Pos(double NumX0)
        {
        //Given an amount of X0, compute the corresponding z position
	  return Cal_First_Pos_m + Cal_Tot_Thickness_m * NumX0/Cal_X0_Per_Layer;
        }
...
std::tuple<double, double, double> DigitizeHit(double z_in, double theta_in, double phi_in,
                                               double z_layer_thickness,
                                               double x_cell_size, double y_cell_size)
{
    // Static RNG with changing seed per call to avoid correlations
    static TRandom3 rng(0);
    static Long64_t call_counter = 0;
    rng.SetSeed(time(nullptr) + (++call_counter));

    // Convert input (z, theta, phi) to Cartesian
    double r = z_in * TMath::Tan(theta_in);
    double x = r * TMath::Cos(phi_in);
    double y = r * TMath::Sin(phi_in);

    // Smearing values
    double sigma_x = x_cell_size / std::sqrt(12.0);
    double sigma_y = y_cell_size / std::sqrt(12.0);
    double sigma_z = z_layer_thickness / std::sqrt(12.0);

    // Apply smearing
    x += rng.Gaus(0, sigma_x);
    y += rng.Gaus(0, sigma_y);
    double z = z_in + rng.Gaus(0, sigma_z);

    // Convert back to (theta, phi, z)
    double r_smear = std::sqrt(x * x + y * y);
    double theta_out = std::atan2(r_smear, z);
    double phi_out = std::atan2(y, x);
    double z_out = z;

    return std::make_tuple(z_out, theta_out, phi_out);
}
...
struct DataLoader{
             ...
             int n_shower_vals = 3)
  {
    ...
    for (size_t j = 0; j < track_tht->size(); ++j) {
      ...
      //Digitize according to detector values for tracker
      std::tie(zval , thtval , phival) = DigitizeHit(zval,thtval,phival,Detector.Input_Tracker.Tracker_Thickness_um*1e-4,Detector.XY_size,Detector.XY_size);      
    ...
    for (size_t j = 0; j < track_tht->size(); ++j) {
      ...
      if (zval <= Detector.Input_Tracker.Tracker_First_Pos_m-0.1 || zval > Cal_2X0 ) continue; // only first 2 X0 of calorimeter AND tracker
      ...
      //if (std::abs(thtval - beamTH) < 0.01745) { //1 degree window
      //  All_Hits.emplace_back(thtval, phival, zval, tval, eneval);
      //}

      //if (std::abs(thtval - beamTH) < 2*0.01745) { //2 degree window
        //All_Hits.emplace_back(thtval, phival, zval, tval, eneval);
	//}

      //      if (std::abs(thtval - beamTH) < 2*0.01745) { //2 degree window

      //Digitize according to detector values for calorimeter or tracker
      if(zval <= Detector.Input_Tracker.Tracker_Last_Pos_m+0.1)
	{
	  //tracker
	  std::tie(zval , thtval , phival) = DigitizeHit(zval,thtval,phival,Detector.Input_Tracker.Tracker_Thickness_um*1e-4,Detector.XY_size,Detector.XY_size);
	}
	...
	{
	  //calorimeter
	  std::tie(zval , thtval , phival) = DigitizeHit(zval,thtval,phival,Detector.Input_Cal.Active_Thickness_um*1e-4,Detector.XY_size,Detector.XY_size);
	}
